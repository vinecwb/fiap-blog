...
export default function CreatePost() {
    ...
    useEffect(() => {
        const userEmail = localStorage.getItem('email'); // Verifique se isso está retornando o email correto
        setUserEmail(userEmail);
        if (!userEmail) {
            console.error('Usuário não está logado');
            return; // Evita prosseguir se o email não estiver disponível
        }
    }, []);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                router.push('/posts'); // Redireciona para a página de posts após a criação do post
            } else {
